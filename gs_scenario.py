...
class Scenario:
    ...
    lists: tuple = None   # feste Vorlieben (pv, po) bei den Lehrbuchkarten cross2 und latin4; sonst None (die Vorlieben kommen aus gs_preferences)
...
def from_points(vehicles, orders, reach, lists=None):
    ...
    return Scenario(tuple(map(tuple, vehicles)), tuple(map(tuple, orders)), int(reach), cost, feasible, lists)
...
def cross2():
    """Zwei Fahrzeuge, zwei Aufträge, gegenläufige Vorlieben: zwei stabile Paarungen (jede Seite bekommt in einer ihre erste Wahl). Die Positionen sind nur Dekoration."""
    return from_points([(20, 30), (60, 30)], [(20, 70), (60, 70)], reach=200, lists=(((0, 1), (1, 0)), ((1, 0), (0, 1))))


def latin4():
    """Knuths 4 x 4-Beispiel (Klein-Gruppe): jede Seite hat ein lateinisches Quadrat als Listen, und es gibt 10 stabile Paarungen (das Maximum für n = 4). Positionen nur Dekoration."""
    pv = ((0, 1, 2, 3), (1, 0, 3, 2), (2, 3, 0, 1), (3, 2, 1, 0))
    po = ((3, 2, 1, 0), (2, 3, 0, 1), (1, 0, 3, 2), (0, 1, 2, 3))
    return from_points([(10 + 25 * i, 30) for i in range(4)], [(10 + 25 * j, 70) for j in range(4)], reach=200, lists=(pv, po))


def road(n=8):
    """Geometrischer Worst Case: Fahrzeuge bei x = 0..n-1, Aufträge bei x = 40..40+n-1 auf einer Geraden, alles in Reichweite. Alle Fahrzeuge bevorzugen denselben (nächsten) Auftrag
    und alle Aufträge dasselbe Fahrzeug: genau n (n + 1) / 2 Vorschläge, für beide Vorschlagenden."""
    return from_points([(i, 50) for i in range(n)], [(40 + j, 50) for j in range(n)], reach=200)


NETS = {
    "steal": lambda: steal_2x2(),
    "p4": lambda: p4_chain(1),
    "cross2": lambda: cross2(),
    "latin4": lambda: latin4(),
    "road": lambda: road(8),
}
